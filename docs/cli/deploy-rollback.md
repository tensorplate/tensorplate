...
1. The CLI checks that `<bundle-path>` exists, is a directory, and contains a
   ...
   any agent call.
2. The bundle path is canonicalized so the agent always sees an absolute
   path.
3. The CLI sends a `ControlOp::Deploy` request and reports the
   `transaction_id` it gets back.
4. In wait mode the CLI polls `ControlOp::Status` until the in-flight
