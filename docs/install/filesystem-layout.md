...
| `/etc/tensorplate/cli.json` | `root:tensorplate` | `0644` | yes | CLI default profile (schema: `config/schemas/cli.json`). The CLI reads it when neither `--config` nor `$TENSORPLATE_CLI_CONFIG` is set. The file mode is world-readable, but the directory above it is not: a caller outside the `tensorplate` group cannot traverse `/etc/tensorplate` and so falls back to the CLI's built-in defaults, with one line on stderr saying so. |
