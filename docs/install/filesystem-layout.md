...
| `/var/lib/tensorplate/state/machine-type.json` | `tensorplate:tensorplate` | `0640` | no | Compute Engine only. Written by `tensorplate-agent`, never by the installer, on every start where the GCE metadata service answered: the machine type, with the kernel boot ID, logical CPU count, `MemTotal` and NVIDIA display device ids it was answered on. Platform detection (agent and `doctor`) reads it only when the metadata service cannot be reached, and uses it only in the same kernel boot while all hardware facts still match exactly. The file survives reboot, but an online agent start must refresh it after each reboot; `doctor` must run as root or as a member of the `tensorplate` group to read it. Removed by purge with the rest of `state/`; the [documented rollback](lifecycle.md#downgrade-and-rollback) restores it from `state.bak` so the older agent keeps it. |
| `/var/lib/tensorplate/identity/instance-binding.json` | `tensorplate:tensorplate` | `0640` | no | Compute Engine only. Written by `tensorplate-agent` beside `machine-type.json` on every start where the metadata service answered: the instance id, the machine type and the kernel boot ID, with the SHA-256 of the `machine-type.json` bytes written in that start. Read on every Compute Engine start, reachable or not, so `doctor` needs the same group membership there to read it. With the service answering, a binding that names another instance fails detection: the disk was moved or cloned, and the host is reprovisioned by deleting both files and starting the agent online. With the service unreachable, a binding written in the same boot must agree with `machine-type.json`, or detection fails. Releases up to 0.2.1 neither read nor write it. Removed by purge. |
