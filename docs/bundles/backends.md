...
| ----------------- | ------------------ | ----------------------------------------------------------------------- |
| `tensorrt`        | Implemented        | Jetson Orin vision validation path, and the only build that compiles the adapter: the released `x86_64` worker and the Homebrew macOS build are both configured `TP_ENABLE_TENSORRT=OFF`. Only the Jetson platform rows declare this path, and only their agent config advertises it. |
