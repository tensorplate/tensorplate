...
This document records the release validation procedure and evidence
...
tensorplate deploy \
 /var/lib/tensorplate/validation/tp-trt-resnet50 \
 --deployment-id validation-trt-resnet50 \
 --output json
tensorplate infer \
...
Observed validation evidence on the connected Jetson target:

- TensorRT built a 49.3 MiB FP16 ResNet50 engine from
 `/usr/src/tensorrt/data/resnet50/ResNet50.onnx`.
- Active deployment `validation-trt-resnet50`, backend `tensorrt`, health
 `ready`.
...
Target setup used for validation:

```bash
python3 -m venv --system-site-packages /var/lib/tensorplate/validation-smolvla-venv
/var/lib/tensorplate/validation-smolvla-venv/bin/python -m pip install \
 -c /var/lib/tensorplate/validation-smolvla-constraints.txt \
 "lerobot[smolvla]==0.4.4"
/var/lib/tensorplate/validation-smolvla-venv/bin/python -m pip install \
 "numpy==1.26.4" "opencv-python-headless==4.10.0.84" \
 "pandas==2.2.3" "Pillow==10.4.0" "scipy==1.11.4"
PYTHONNOUSERSITE=1 /var/lib/tensorplate/validation-smolvla-venv/bin/python -m pip install \
...
```bash
TP_PYTHON_PYTORCH_EXECUTABLE=/var/lib/tensorplate/validation-smolvla-venv/bin/python
...
```bash
/var/lib/tensorplate/validation-smolvla-venv/bin/python \
...
tensorplate deploy \
 /var/lib/tensorplate/validation/tp-smolvla-real \
 --deployment-id validation-smolvla-real \
 --output json
tensorplate infer \
...
- Active deployment `validation-smolvla-real`, backend `python_pytorch`, model
...
- `tensorplate doctor --output json` reported `failing: 0` with active
 deployment `validation-smolvla-real`.

## Current Gaps To Track

- The checked-in `test/models/bundles/v0_1/vision_tensorrt` fixture is
 parser-only and is not functional TensorRT evidence.
- The checked-in `smolvla_python_pytorch` fixture is not a real SmolVLA
 policy. Use it only for bundle/parser compatibility; the real SmolVLA
 evidence is the `validation-smolvla-real` sidecar validation above.
