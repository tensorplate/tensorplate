...
6. File the report, its stage logs and the recorded row facts under
   ...
   **Sanitize before the first commit** — see that directory's README —
   ...
   | `offline-agent-journal.txt` | raw journal records from the agent instance that ran under the denial, with the same host metadata fields as the other journal captures |
