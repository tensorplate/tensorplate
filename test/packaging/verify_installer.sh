...
# fatal only under --strict-hardware.
TP_INSTALL_NV_TEGRA_RELEASE="${td}/absent-nv-tegra" \
TP_INSTALL_OS_RELEASE="${td}/os-release.noble" \
TP_INSTALL_NVIDIA_VERSION="${td}/absent-nvidia-version" \
TP_INSTALL_ARCH="x86_64" \
TP_INSTALL_DEB_ARCH="amd64" \
  bash "${installer}" --dry-run --yes >"${td}/x86-nogpu.out" 2>"${td}/x86-nogpu.err"
...
if TP_INSTALL_NV_TEGRA_RELEASE="${td}/absent-nv-tegra" \
   ...
   TP_INSTALL_ARCH="x86_64" \
   TP_INSTALL_DEB_ARCH="amd64" \
     bash "${installer}" --dry-run --strict-hardware --yes >"${td}/x86-strict.out" 2>"${td}/x86-strict.err"; then
