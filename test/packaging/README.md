...
| `verify_macos_offline_runtime.sh` | Tests `tools/validation/macos_offline_runtime.py` and the harness's `offline-profile` and `offline-runtime` stages. On every host: profile rendering and its independent lint against mutants, plist derivation refusals, `launchctl print` and `lsof -F` parsing, the `sandbox_check` identity check and discrimination controls, classification of probe outcomes recorded under the rendered profile and seven mutants, and the doctor, status, deploy, inference and admission checks. Runs the extracted offline-runtime stage and cleanup against a fake Homebrew, launchd, process table and CLI (`fixtures/macos-offline/fake_host.py`) through a clean run, one run per injected fault, purges from partial launchd states, and INT, TERM and HUP delivered mid-stage and again during the restore, requiring a fail row and normal launchd supervision afterwards. Copies of the harness with one guard removed (a `run_denied` wrapper, the stopped-services flag, the cleanup restore) must fail; under macOS `/bin/bash` 3.2 so must copies without the TERM or HUP trap, without signal ignoring during the restore, or without the terminal restore. What CI proves is the guard logic, classification of recorded outcomes and the signal tests under bash 5; the SBPL semantics and the bash 3.2 behaviour those signal guards exist for are proven only where the verifier runs on macOS, where it also runs the preflight against the real `sandbox-exec` with every mutant, and on the M1 hardware run. Every probe socket is pinned to `lo0`, so nothing leaves the host. |
