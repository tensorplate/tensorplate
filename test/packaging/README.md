...
V01-E14-F08 packaging verification suite.
...
- Actual `.deb` artifact build (`dpkg-buildpackage`). The skeleton is
  ...
  staged on host CI yet — `dpkg-buildpackage` runs on the V01-E15
  Jetson target.
- Live `systemctl enable --now`. That happens on the V01-E15 target
...
- CUDA / TensorRT / LibTorch functional checks. F06 doctor probes
  cover artifact presence; functional validation belongs to V01-E15.
