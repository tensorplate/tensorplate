...
| `verify_jetson_lifecycle.sh` | Syntax-checks and shellchecks the native Jetson lifecycle harness `tools/validation/jetson-lifecycle.sh`, checks that it covers the eight canonical stages with reasoned skips and records the artifact digest after install, and that its bundle staging path is visible to the agent unit's sandbox. Drives the preflight refusals through seams (architecture, Ubuntu 22.04, L4T R36, tag and manifest binding, checksums, bundle build before purge) and every exercised stage against a stubbed appliance with injected failures, so each stage is shown to record `fail` when its assertion fails, including crash-loop config restoration on signals. The hardware run itself is manual. |
