...
inline NamedInput make_named_input(std::string name, std::uint64_t buffer_id = 1,
  ...
  auto view = TensorView::create(DType::Float32, {1, static_cast<std::int64_t>(byte_size / 4)},
...
inline SchedulerRequest make_scheduler_request(InferRequest request, const SchedulerClock& clock,
...
inline SchedulerRequest make_request_with_owned_buffer(
    ...
    std::size_t byte_size = 64, std::optional<InferRequest::TimePoint> deadline = std::nullopt) {
  ...
  auto view_r = TensorView::create(DType::Float32, {1, static_cast<std::int64_t>(byte_size / 4)},
                                   Layout::RowMajor, 0, byte_size);
  if (!view_r) {
    throw std::runtime_error("test fixture: TensorView::create failed: " + view_r.error().message);
  }
  auto view = std::move(view_r).value();
  std::vector<NamedInput> inputs;
  inputs.push_back(NamedInput{"input", buf, view});
  auto req_r =
      InferRequest::create(std::move(request_id), "test/endpoint", std::move(inputs), {}, deadline);
  if (!req_r) {
    throw std::runtime_error("test fixture: InferRequest::create failed: " + req_r.error().message);
  }
