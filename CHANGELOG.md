...
- `protocol/schemas/worker_control.json` gains the runtime control
  ...
  active, closing and ceiling counts plus the admission time of each
  reserved or active session) and `pressure_directive` (`normal`,
  `shed_admission`, `terminate_newest` with a count, `resume`) use the
  literal transaction id `runtime`. Every runtime request carries a
  correlation id and names the member (deployment and generation) it is
  for; every runtime answer echoes both ids and names the member that
  answered, and a request for another generation is answered with the new
  `member_mismatch` status. An `ok` answer to `activate` or `quota_assign`
  carries the quota in force; a failed answer carries its error, and no
  other answer does. Both ids follow the shared `[A-Za-z0-9_-]{1,64}`
  policy on runtime frames. The agent repeats only its latest request on a
  channel, with the same correlation id, and the worker answers a repeat
  from its record of that request instead of applying it again. A member
  holds at most 2048 sessions, in the state file's quota too, so the
  largest ledger answer fits one frame. Frames are compact JSON, one per
  line, at most 65536 bytes; the golden frames under
  `protocol/rust/tests/fixtures/worker_control_*.jsonl` pin the exact bytes
  for other implementations of the channel. The legacy operations stay in
