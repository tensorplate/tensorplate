...
pub struct AgentStub {
    ...
    raw_history: Arc<Mutex<Vec<String>>>,
    stop: Arc<AtomicBool>,
...
impl AgentStub {
    pub fn start() -> Self {
        ...
        let r_thread = raw_history.clone();
        let thread = std::thread::spawn(move || {
            accept_loop(&listener, &stop_thread, &q_thread, &h_thread, &r_thread);
        });
        Self {
            td,
            socket,
    ...
    /// Every request line exactly as the CLI sent it, without its newline.
    pub fn raw_history(&self) -> Vec<String> {
        self.raw_history.lock().expect("lock").clone()
    }
    ...
    while !stop.load(Ordering::Relaxed) {
        match listener.accept() {
            Ok((stream, _addr)) => {
                ...
                handle_connection(stream, queue, history, raw_history);
            }
